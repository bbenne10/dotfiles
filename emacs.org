...
* Set up notmuch
  ...
  (use-package notmuch
    ...
    :if (file-exists-p "/usr/local/share/emacs/site-list/notmuch/")
    :init
...
* Fix up hideshow mode
  ...
  #+end_src
* Maybe start using exwm...who knows?
...
#+begin_src elisp :tangle ~/.emacs.d/packages.el
  (use-package exwm
    :config
      ...
      (defun bb-exwm-bind-command (key command &rest bindings)
        (while key
      ...
      ;; by default every buffer created by exwm is named '*EXWM*'.
      ;; This makes it hard to know which buffer to select.
      ;; Below, we change that behavior to use the instance name instead
      ...
      (exwm-input-set-key (kbd "s-r") #'exwm-reset) ;; reset exwm
      (exwm-input-set-key (kbd "s-w") #'exwm-workspace-switch) ;; interactively switch workspaces
  
      ;; set up the bindings for switching workspaces directly
      (dotimes (i 4)
      ...
      (bb-exwm-bind-command "<s-S-return>"  "st")
      (bb-exwm-bind-command "<C-s-SPC>" "~/.bin/lock_screen")
      (bb-exwm-bind-command "<XF86AudioPlay>" "mpc toggle")
      (bb-exwm-bind-command "<XF86AudioNext>" "mpc next")
      (bb-exwm-bind-command "<XF86AudioPrev>" "mpc prev")
      (bb-exwm-bind-command "<s-p>" "dmenu_run -fn Hasklig:10 -x .225 -y 25 -w .5")
      (bb-exwm-bind-command "<XF86AudioLowerVolume>" "pamixer set-sink-volume 0 -5%")
      (bb-exwm-bind-command "<XF86AudioRaiseVolume>" "pamixer set-sink-volume 0 +5%")
      (bb-exwm-bind-command "<XF86AudioRaiseMute>" "pamixer set-sink-mute 0 toggle")
  
      (exwm-input-set-key (kbd (concat default-leader-key "-" "b")) #'ivy-switch-buffer)
